...
const FilterModal: React.FC<FilterModalProps> = ({
    ...
    const [gradoEscolaridadOptions, setGradoEscolaridadOptions] = useState<string[]>([]);
    ...
    useEffect(() => {
        const uniqueGrados = Array.from(new Set(estudiantes.map(student => student.gradoEscolaridad)));
        setGradoEscolaridadOptions(uniqueGrados);
    }, [estudiantes]);

    const handleCheckboxChange = (value: string) => {
        setFilters((prevFilters) => {
            const updatedGradoEscolaridad = prevFilters.gradoEscolaridad.includes(value)
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleClose}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-center text-2xl">Filtros de Búsqueda</DialogTitle>
                </DialogHeader>
                <Accordion type="multiple" className="space-y-4 mt-4">
                    {/* Grado Escolaridad Filters */}
                    <AccordionItem value="gradoEscolaridad">
                        <AccordionTrigger className="text-lg font-semibold">Grado Escolaridad</AccordionTrigger>
                        <AccordionContent>
                    ...
                    <AccordionItem value="fechaProximoSeguimiento">
                        <AccordionTrigger className="text-lg font-semibold">Fecha Próximo Seguimiento</AccordionTrigger>
                        <AccordionContent>
                            <DatePickerWithRange
